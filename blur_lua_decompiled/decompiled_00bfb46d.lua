...
function PostInit()
  ...
  LSP.GetTop20Leaderboard(1, false)
  GUI.doing_photo_task = true
  for _FORV_3_ = 1, GUI.num_items do
    UIButtons.AddListItem(GUI.node_list_id, UIButtons.CloneXtGadgetByName("Photos\\Top20DownloadedPhotos.lua", "_node"), _FORV_3_ - 1)
    GUI.ids[#GUI.ids + 1] = {
      node = UIButtons.CloneXtGadgetByName("Photos\\Top20DownloadedPhotos.lua", "_node"),
      text = UIButtons.FindChildByName(UIButtons.CloneXtGadgetByName("Photos\\Top20DownloadedPhotos.lua", "_node"), "_text")
    }
  end
end
...
function MessageUpdate(_ARG_0_, _ARG_1_, _ARG_2_, _ARG_3_, _ARG_4_)
  ...
  elseif _ARG_0_ == UIEnums.Message.MenuNext and _ARG_2_ == true then
    ...
    GUI.doing_photo_task = true
    LSP.GetMetaDataFromLeaderboardIndex(GUI.leaderboardIndex, false)
    GUI.PhotoTask = GUI.PhotoTaskGetMetaData
  elseif _ARG_0_ == UIEnums.Message.ButtonX and _ARG_2_ == true then
    Amax.ViewPhotoTop20GamerCard(Profile.GetPrimaryPad(), UIButtons.GetSelection(GUI.node_list_id), false)
  end
end
  ...
  if GUI.doing_photo_task == true then
    ...
    if still_pumping == false then
      ...
      if GUI.PhotoTask == GUI.PhotoTaskLeaderboardFinished then
        ...
        if LSP.IsStatsConnected() == false or #LSP.GetReturnedLeaderboardData(false) == 0 then
          SetupCustomPopup(UIEnums.CustomPopups.ContentServerLeaderboardError)
        else
          for _FORV_5_, _FORV_6_ in ipairs((LSP.GetReturnedLeaderboardData(false))) do
            UIButtons.SetNodeItemLocked(GUI.node_list_id, _FORV_5_ - 1, false)
            UIButtons.ChangeText(GUI.ids[_FORV_5_].text, "GAME_PHOTO_LEADERBOARD_DOWNLOAD_" .. _FORV_6_.index)
          end
          for _FORV_5_ = #LSP.GetReturnedLeaderboardData(false) + 1, #GUI.ids do
      ...
      elseif GUI.PhotoTask == GUI.PhotoTaskFinished then
        ...
        if error ~= 0 then
          ...
          GUIBank.photo_viewing_from = GUIBank.PhotoViewFrom.Top20Download
