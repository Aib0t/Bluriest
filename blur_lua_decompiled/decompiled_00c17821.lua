...
function Init()
  ...
  SetupMenuOptions()
  SetupCustomPopup(UIEnums.CustomPopups.TwitterLegal)
end
function PostInit()
end
...
function MessageUpdate(_ARG_0_, _ARG_1_, _ARG_2_, _ARG_3_, _ARG_4_)
  ...
  if _ARG_0_ == UIEnums.Message.MenuBack and _ARG_2_ == true or getMouseButton(_ARG_0_, 0, _ARG_1_, _ARG_3_) == UIEnums.Message.ButtonB then
    PlaySfxBack()
    if UIGlobals.TwitterBackToTweetScreen == false then
      UIScreen.EndScreen()
    else
  ...
  if _ARG_0_ == UIEnums.Message.MenuNext and _ARG_2_ == true then
    if UIEnums.CurrentPlatform == UIEnums.Platform.PC then
      ...
      elseif GUI.option == 2 then
        GUI.timeout = 0
        SetupCustomPopup(UIEnums.CustomPopups.TwitterAuthenticating)
        Amax.TwitterAuthenticate()
        GUI.authenticating = true
      end
    elseif GUI.option == 0 then
      UIHardware.StartKeyboard(_ARG_1_, "GAME_TWITTER_USERNAME", UIText.RBC_TWITTER_KEYB_HEADER, UIText.RBU_TWITTER_KEYB_USERNAME, 256, UIEnums.XboxKeyboardType.Standard)
    elseif GUI.option == 1 then
      UIHardware.StartKeyboard(_ARG_1_, UIText.CMN_NOWT, UIText.RBC_TWITTER_KEYB_HEADER, UIText.RBU_TWITTER_KEYB_PASSWORD, 256, UIEnums.XboxKeyboardType.Password)
    elseif GUI.option == 2 then
      GUI.timeout = 0
      SetupCustomPopup(UIEnums.CustomPopups.TwitterAuthenticating)
      Amax.TwitterAuthenticate()
  ...
  elseif _ARG_0_ == UIEnums.Message.KeyboardFinished then
    if GUI.option == 0 then
      Amax.StoreTwitterUsername()
      UIButtons.ChangeText(SCUI.name_to_id.username_text, "GAME_TWITTER_USERNAME")
    elseif GUI.option == 1 then
      Amax.StoreTwitterPassword()
      UIButtons.ChangeText(SCUI.name_to_id.password_text, "GAME_TWITTER_PASSWORD_ASTERISK")
    end
  end
end
function FrameUpdate(_ARG_0_)
  ...
  if GUI.BlockInput == true then
    ...
    if UIButtons.GetUserInputString(40) == nil then
      ...
      if GUI.CurrentSelectId == GUI.UsernameID then
        Amax.StoreTwitterUsername((UIButtons.GetUserInputString(40)))
        UIButtons.ChangeText(GUI.CurrentSelectId, "GAME_TWITTER_USERNAME")
      else
        Amax.StoreTwitterPassword((UIButtons.GetUserInputString(40)))
        UIButtons.ChangeText(GUI.PasswordID, "GAME_TWITTER_PASSWORD_ASTERISK")
      end
  ...
  if GUI.authenticating == true then
    ...
    if GUI.timeout > 2 then
      still_pumping, error = Amax.PumpTwitterCurrentTask()
      ...
      if still_pumping == false then
        ...
        elseif error ~= 0 then
          ...
          PopupSpawn(UIEnums.CustomPopups.TwitterUserUnknown)
        else
          PopupSpawn(UIEnums.CustomPopups.TwitterAuthenticated)
        end
      end
    end
  elseif GUI.option == 0 then
